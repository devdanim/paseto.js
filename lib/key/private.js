...
function generate(cb) {
  ...
  return sodium.ready.then(() => {
    ...
    return self.inject(Buffer.from(sodium.crypto_sign_keypair().privateKey), (err) => { return done(err); });
  });
}
...
function _public(cb) {
  ...
  const rkey = Buffer.from(sodium.crypto_sign_ed25519_sk_to_pk(self.raw()));
